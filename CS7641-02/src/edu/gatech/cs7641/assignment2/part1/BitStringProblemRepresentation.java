...
public class BitStringProblemRepresentation implements HillClimbingProblem,
	...
	public BitStringProblemRepresentation(int size, Random random,
		...
		for (int i=0; i<size; i++) range[i]=2;
		this.dist = new DiscreteUniformDistribution(range);
	}

	@Override
	public double value(Instance d) {
		if (d.size()!=this.size) throw new IllegalArgumentException("Instance size mismatch");
		return eval.value(d);
	}
	...
	public Instance mate(Instance a, Instance b) {
		if (a.size()!=this.size) throw new IllegalArgumentException("Instance size mismatch");
		if (b.size()!=this.size) throw new IllegalArgumentException("Instance size mismatch");
